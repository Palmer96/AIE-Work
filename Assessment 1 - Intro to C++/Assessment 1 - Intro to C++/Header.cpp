...
{
	for (int i = 0; i <= m_Length; i++)			// loop until end of m_str
	{
...
{
	char* temp = new char[Length(str) + m_Length + 1];		// create a temp array the size of the current length + the input being added
	...
	{
		temp[i + m_Length] = str[i];			// add the input to the end of the temp
	}
	delete[] m_str;								// delete what m_str points too
	m_str = temp;								// points m_str to temp			 
	m_Length = Length(temp);					// change size of m_Length 					
}
//-------------------------------------------------------------------------------- Prepend() 
void String::Prepend(char str[])
{
	char* temp = new char[Length(str) + m_Length + 1];		// create a temp array the size of the current length + the string being added

	for (int i = 0; i <= Length(str); i++)		// loop until end of m_str
	{
		temp[i] = str[i];						// make the temp array = m_str
	}
	for (int i = 0; i <= m_Length; i++)			// loop until end of input
	{
...
int String::Find(int startIndex, char findString[])
{
	...
	for (int i = startIndex; i <= m_Length; i++)	// loop until end of m_str, starting at startIndex
	{
		int j;
		for (j = 0; m_str[i + j] == findString[j]; j++)
		{
...
void String::Replace(char findString[], char replaceString[])
{
	while (Find(findString) != -1)				// run and loop if able to find string
	{
		...
		int tempLength = m_Length + (rSize - fSize) + 1;	// create a temp large enough for end result
		...
		if (rSize != fSize)						// check if the replace string and find string are the same 
		{
			...
			delete[] m_str;						// delete what m_str points too
			m_str = temp;						// points m_str to temp				
			m_Length = tempLength;				// change size of m_Length 
